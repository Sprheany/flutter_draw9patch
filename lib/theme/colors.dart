// ignore_for_file: constant_identifier_names, non_constant_identifier_names

import 'package:flutter/painting.dart';

final int BLACK_TICK = const Color.fromARGB(255, 0, 0, 0).value;
final int RED_TICK = const Color.fromARGB(255, 255, 0, 0).value;

const Color CORRUPTED_COLOR = Color.fromARGB(179, 255, 0, 0);
const Color LOCK_COLOR = Color.fromARGB(179, 0, 0, 0);
const Color STRIPES_COLOR = Color.fromARGB(128, 255, 0, 0);
const Color PATCH_COLOR = Color.fromARGB(128, 255, 94, 252);
const Color PATCH_ONEWAY_COLOR = Color.fromARGB(128, 94, 255, 94);
const Color HIGHLIGHT_REGION_COLOR = Color.fromARGB(128, 128, 128, 128);

const Color PADDING_COLOR = Color.fromARGB(128, 94, 94, 255);

const Color BLUE_COLOR = Color.fromARGB(255, 0, 0, 255);
const Color WHITE_COLOR = Color.fromARGB(255, 255, 255, 255);
